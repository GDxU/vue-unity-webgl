...
export default {
  props: ['src', 'module', 'width', 'height', 'externalProgress', 'unityPrefix', 'hideFooter'],
  ...
  beforeMount() {
    ...
    if (typeof createUnityInstance === 'undefined' && this.unityLoader && !this.eventBus.load) {
  ...
  mounted() {
    const instantiate = () => {
      ...
      let promise = createUnityInstance(document.querySelector(this.unityCanvas), {
